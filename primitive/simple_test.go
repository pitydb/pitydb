...
func TestTableMeta(t *testing.T) {
	...
	slot0 := NewCellMetaRaw(0, PTypeUInt32, "id", "the auto incrementID", nil)
	slot1 := NewCellMetaRaw(1, PTypeInt64, "col1", "col1...", int64(-1000))
	slot2 := NewCellMetaRaw(2, PTypeBool, "col2", "col2...", false)
	slot3 := NewCellMetaRaw(3, PTypeByte, "col3", "col3...", byte(0xff))
	slot4 := NewCellMetaRaw(4, PTypeFloat64, "col4", "col4...", float64(0.9999999))
	slot5 := NewCellMetaRaw(5, PTypeString, "col5", "col5...", "very very pity!")

	tableMeta.AddMetaRowItem(slot2)

	tableMeta.AddMetaRowItem(slot0)
	tableMeta.AddMetaRowItem(slot1)
	tableMeta.AddMetaRowItem(slot2)
	tableMeta.AddMetaRowItem(slot3)
	tableMeta.AddMetaRowItem(slot5)
	tableMeta.AddMetaRowItem(slot4)

	if 6 != tableMeta.GetCellSize() {
		t.Fatal("table size should be 6, but:", tableMeta.GetCellSize())
	}

	for _, item := range tableMeta.GetItems() {
		println(item.GetComment())
	}
}

func TestDefaultIndexRowMeta(t *testing.T) {
	rMeta := DefaultIndexRowMeta()
	if PTypeUInt32 != rMeta.GetItems()[0].GetMType() {
